...
import { Plus, Camera, Wifi, WifiOff, Wrench, Trash2, Edit2, Check, X } from 'lucide-react';
...
export default function CamerasPage() {
  ...
  const [editingStream, setEditingStream] = useState(null); // cameraId being edited
  ...
  const handleSaveStream = async (cameraId) => {
    try {
      const res = await cameraAPI.update(cameraId, { streamUrl: editStreamVal });
      setCameras(prev => prev.map(c => c.cameraId === cameraId ? res.data.camera : c));
      setEditingStream(null);
      toast.success('Stream URL updated — AI service will use this on next start');
    } catch { toast.error('Failed to update stream URL'); }
  };
  ...
  return (
    <div>
      ...
      <div className="page-body" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* Info banner */}
        <div style={{ background: 'var(--accent-dim)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 8, padding: '10px 16px', fontSize: 12, color: 'var(--accent)' }}>
          💡 Set each camera's Stream URL here — the AI service reads it automatically on startup. No need to edit any config files.
        </div>
        ...
        <div className="grid-3">
          {cameras.map(cam => {
            ...
            const isEditingThis = editingStream === cam.cameraId;
            return (
              <div key={cam._id} className="card card-sm" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {/* Stream URL editor */}
                <div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5, fontWeight: 500 }}>Stream URL</div>
                  {isEditingThis ? (
                    <div style={{ display: 'flex', gap: 6 }}>
                      <input
                        ...
                        autoFocus
                      />
                      <button className="btn btn-primary btn-sm" onClick={() => handleSaveStream(cam.cameraId)}><Check size={12} /></button>
                      <button className="btn btn-ghost btn-sm" onClick={() => setEditingStream(null)}><X size={12} /></button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 11, color: cam.streamUrl ? 'var(--text-secondary)' : 'var(--text-muted)', fontFamily: 'var(--mono)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {cam.streamUrl || 'Not set'}
                      </span>
                      {isSupervisor && (
                        <button className="btn btn-ghost btn-sm" style={{ padding: '3px 7px', flexShrink: 0 }}
                          onClick={() => { setEditingStream(cam.cameraId); setEditStreamVal(cam.streamUrl || ''); }}>
                          <Edit2 size={11} />
                        </button>
                      )}
                    </div>
            ...
            <form onSubmit={handleAdd}>
              <div className="modal-body">
                {[
                  ...
                  { key: 'streamUrl', label: 'Stream URL', placeholder: 'http://192.168.1.x:4747/video or rtsp://...' },
                ].map(f => (
                  <div key={f.key} className="form-group">
                    <label>{f.label}{f.key !== 'streamUrl' && ' *'}</label>
